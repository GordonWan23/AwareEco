...
label21 = tk.Label(newWindow1, text='Mode:Sorting Glass')
label21.config(font=('helvetica', 24), foreground="Blue")
...
def sortContour(contour):
    ...
    for box in cnt_t_rec:
        ...
        for store in contourList:
            diff = abs(
                abs(box[0] - store[0]) + abs(box[1] - store[1]) + abs(box[2] - store[2]) + abs(box[3] - store[3]))
            if diff <= 80:
...
def findStable(x, y, h, w, i, area):
    ...
    if (x - x1_ <= 20) and (y - y1_ <= 20) and (h - h1_ <= 20) and (w - w1_ <= 20) and (
            x1_ >= 0 and x >= 0 and y1_ >= 0 and y >= 0):
...
def timeout():
    ...
    for items in categoryList:
        if items == "glass" and flagRecyc == 0:
            print("Unrecyclable")
            flagRecyc = 1
            if len(categoryList) > 1:
                label23 = tk.Label(newWindow1, text='Too Many Items,Found Glass')
                label23.config(font=('helvetica', 18), foreground="Red")
                canvas2.itemconfigure(l23, window=label23)
                trashinGroup = 1
                warnCount -= 5
            else:
                label23 = tk.Label(newWindow1, text='Sorting Glass')
    ...
    if flagRecyc == 0:
        print("Recyclable")
        if len(categoryList) > 1:
            label23 = tk.Label(newWindow1, text='Too Many Items,No Glass')
            label23.config(font=('helvetica', 18), foreground="Red")
            canvas2.itemconfigure(l23, window=label23)
            trashnotinGroup = 1
            warnCount -= 5
